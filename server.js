...
async function api(req, res, pathname) {
  ...
  if (recipeMatch && req.method === 'PUT') { const index = state.recipes.findIndex(item => item.id === decodeURIComponent(recipeMatch[1])); if (index < 0) return json(res, 404, { error: 'Recette introuvable.' }); if (!state.recipes[index].personal) return json(res, 403, { error: 'Les recettes de saison ne sont pas modifiables.' }); const recipe = recipePayload(await readBody(req), state.recipes[index]); if (!recipe.title || !recipe.ingredients.length || !recipe.preparation.length) return json(res, 400, { error: 'Titre, ingrédients et préparation sont requis.' }); state.recipes[index] = recipe; recalculateShopping(state); writeState(state); return json(res, 200, recipe); }
  if (recipeMatch && req.method === 'DELETE') { const index = state.recipes.findIndex(item => item.id === decodeURIComponent(recipeMatch[1])); if (index < 0) return json(res, 404, { error: 'Recette introuvable.' }); if (!state.recipes[index].personal) return json(res, 403, { error: 'Les recettes de saison ne sont pas supprimables.' }); const id = state.recipes[index].id; state.recipes.splice(index, 1); for (const meals of Object.values(state.menu)) { if (meals.lunch === id) meals.lunch = null; if (meals.dinner === id) meals.dinner = null; } recalculateShopping(state); writeState(state); return json(res, 204, null); }
