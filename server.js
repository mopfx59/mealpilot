...
const defaultState = () => ({ recipes: seasonalRecipes(), menu: Object.fromEntries(DAYS.map(day => [day, { lunch: null, dinner: null }])), plan: null, leftovers: [], shopping: [], providers: structuredClone(DEFAULT_PROVIDERS), providerStatus: {}, recipeCache: {}, calendar: { connected: false, calendarId: 'primary', calendarName: 'Agenda principal', events: [], schoolHolidays: [], lastSyncAt: null } });
...
function migrateState(state) {
  ...
  state.shopping ||= [];
  state.plan ||= null; state.leftovers ||= []; state.providers ||= {}; for (const [key, provider] of Object.entries(DEFAULT_PROVIDERS)) state.providers[key] = { ...structuredClone(provider), ...(state.providers[key] || {}) }; state.providerStatus ||= {}; state.recipeCache ||= {};
  ...
  for (const recipe of state.recipes) {
    ...
    recipe.personal ??= true; recipe.season ||= 'Toute saison'; recipe.seasons = recipeSeasons(recipe); recipe.category ||= 'Autre'; recipe.favorite = Boolean(recipe.favorite); recipe.leftoverFriendly ??= !/salade|tartare|soufflé|friture/i.test(`${recipe.title} ${recipe.category}`);
  }
...
async function api(req, res, pathname, url) {
  ...
  const leftoverMatch = pathname.match(/^\/api\/leftovers\/([^/]+)$/);
  if (leftoverMatch && req.method === 'PATCH') { const leftover = state.leftovers.find(item => item.id === decodeURIComponent(leftoverMatch[1])); if (!leftover) return json(res, 404, { error: 'Reste introuvable.' }); const body = await readBody(req); if (!['consumed', 'conserved', 'discarded'].includes(body.status)) return json(res, 400, { error: 'Statut de reste invalide.' }); leftover.status = body.status; if (body.status === 'consumed' || body.status === 'discarded') leftover.remainingServings = 0; else if (Number.isFinite(Number(body.remainingServings))) leftover.remainingServings = Math.max(0, Number(body.remainingServings)); leftover.updatedAt = new Date().toISOString(); writeState(state); return json(res, 200, leftover); }
