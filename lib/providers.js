...
class RecipeCollector {
  ...
  async importUrl(value, state) {
    let target;
    try { target = new URL(String(value || '').trim()); } catch { throw new Error('Adresse de recette invalide.'); }
    if (target.protocol !== 'https:') throw new Error('Seules les adresses HTTPS sont acceptées.');
    if (target.username || target.password) throw new Error('Cette adresse contient des identifiants et ne peut pas être utilisée.');
    const host = target.hostname.toLowerCase(); const privateIpv4 = /^(?:10\.|127\.|169\.254\.|192\.168\.|172\.(?:1[6-9]|2\d|3[01])\.)/;
    if (host === 'localhost' || host.endsWith('.local') || host.endsWith('.internal') || (isIP(host) === 4 && privateIpv4.test(host)) || (isIP(host) === 6 && /^(?:::1|f[cd]|fe[89ab])/i.test(host))) throw new Error('Les adresses locales ou privées ne sont pas acceptées.');
    const source = { key: `url:${host}`, name: host.replace(/^www\./, ''), minIntervalMs: 0, timeoutMs: 10000 };
    if (!await this.allowed(target.href, source, state)) throw new Error('Cette page interdit la collecte automatique dans robots.txt.');
    const page = await this.cachedRequest(target.href, source, state, 'recipe');
    const recipes = jsonLdBlocks(page).flatMap(findRecipes).map(raw => normalizeRecipe(raw, source, target.href)).filter(Boolean);
    if (!recipes.length) throw new Error('Aucune recette structurée compatible n’a été trouvée sur cette page.');
    return recipes[0];
  }
