...
function extractLinks(html, baseUrl, hosts) {
  const links = new Set(); const regex = /<a\b[^>]*href=["']([^"'#]+)["']/gi; let match;
  while ((match = regex.exec(html)) && links.size < 12) { try { const url = new URL(decode(match[1]), baseUrl); if (hosts.some(host => url.hostname === host || url.hostname.endsWith(`.${host}`)) && /recette/i.test(url.pathname)) links.add(url.href); } catch {} }
  return [...links];
}
  ...
  async search(query, state, wanted = 6) {
    ...
    for (const [key, base] of Object.entries(state.providers || DEFAULT_PROVIDERS)) {
      ...
      try {
        ...
        const urls = extractLinks(searchHtml, searchUrl, source.hosts);
        for (const url of urls.slice(0, Math.max(2, wanted - results.length))) {
          ...
          for (const raw of jsonLdBlocks(page.html).flatMap(findRecipes)) { const recipe = normalizeRecipe(raw, source, url); if (recipe && !results.some(item => item.fingerprint === recipe.fingerprint)) results.push(recipe); if (results.length >= wanted) break; }
...
module.exports = { DEFAULT_PROVIDERS, RecipeCollector, jsonLdBlocks, findRecipes, normalizeRecipe, fingerprint, canonical };
