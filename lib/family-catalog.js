...
function familyCatalog() {
  const additions = entries.map(([title,totalMinutes,ingredients,preparation]) => ({ id: `family-${randomUUID()}`, title, description: 'Une recette familiale simple, avec des ingrédients courants.', totalMinutes, prepMinutes: Math.min(20,totalMinutes), servings: 4, ingredients, preparation, source: 'Recette originale MealPilot', personal: false, favorite: false }));
