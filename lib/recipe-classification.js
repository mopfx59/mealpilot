...
function inferCategory(recipe) {
  ...
  if (/\b(lasagne|spaghetti|tagliatelle|ravioli|gnocchi|pates?|pasta|one pot)\b/.test(title)) return 'Pâtes';
...
function classifyRecipe(recipe) {
  ...
  return { category, seasons, season: seasons.length === 1 ? seasons[0] : seasons[0], classificationVersion: 2 };
}
