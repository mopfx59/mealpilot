...
function score(recipe, { season, quick, recent, index }) { let value = recipe.season === season ? 50 : recipe.season === 'Toute saison' ? 15 : 0; if (quick && Number(recipe.totalMinutes || recipe.prepMinutes || 999) <= 30) value += 35; value -= (recent.get(recipe.id) || 0) * 60; value -= Math.abs((index % 7) - ((recipe.title || '').length % 7)); return value; }
...
function generatePlan(state, { startDate, endDate, soloMadame = [] }) {
  ...
  dates.forEach((date, dayIndex) => ['lunch', 'dinner'].forEach((meal, mealIndex) => {
    ...
    if (!recipe) { const ranked = recipes.map((item, index) => ({ item, value: score(item, { season: seasonFor(date), quick: meal === 'dinner' || attendance.afterNight, recent, index: dayIndex * 2 + mealIndex + index }) })).sort((a, b) => b.value - a.value); recipe = ranked[0].item; recent.set(recipe.id, (recent.get(recipe.id) || 0) + 1); if (!isSolo && recipe.leftoverFriendly !== false && (dayIndex + mealIndex) % 3 === 0) leftover = { recipe, uses: 1 }; }
