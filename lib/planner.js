...
function generatePlan(state, { startDate, endDate, soloMadame = [], expressDates = [] }) {
  ...
  const nextLeftoverNeed = (recipe, currentDayIndex, currentMealIndex) => {
    for (let futureDayIndex = currentDayIndex; futureDayIndex < dates.length; futureDayIndex += 1) {
      for (let futureMealIndex = 0; futureMealIndex < 2; futureMealIndex += 1) {
        ...
        if (futureSolo || futureMeal === 'lunch') return futureSolo ? 1 : futureAttendance.servings;
      }
    }
    return 0;
  };
  dates.forEach((date, dayIndex) => ['lunch', 'dinner'].forEach((meal, mealIndex) => {
    ...
    const expressRequired = isSolo || expressDates.includes(key) || attendance.afterNight; let expressSelected = false;
    ...
    const entry = { id: randomUUID(), date: key, meal, recipeId: recipe.id, servings, cookedServings: servings, attendance, isSoloMadame: isSolo, express: expressSelected, fromLeftover, leftoverId };
    if (!fromLeftover && !plannedLeftover && !isSolo && recipe.leftoverFriendly !== false && (dayIndex + mealIndex) % 3 === 0) { const extraServings = nextLeftoverNeed(recipe, dayIndex, mealIndex); if (extraServings > 0) { entry.cookedServings += extraServings; const record = { id: randomUUID(), recipeId: recipe.id, sourceMealId: entry.id, sourceDate: key, targetServings: extraServings, initialServings: extraServings, remainingServings: extraServings, status: 'conserved', createdAt: new Date().toISOString() }; state.leftovers.push(record); plannedLeftover = { recipe, remainingServings: extraServings, record }; } }
    meals.push(entry);
  }));
