...
function score(recipe, { season, maxPrepMinutes, recent, index }) { const duration = Number(recipe.totalMinutes || recipe.prepMinutes || 999); let value = recipe.season === season ? 50 : recipe.season === 'Toute saison' ? 15 : 0; if (Number.isFinite(maxPrepMinutes)) value += duration <= maxPrepMinutes ? 45 : -Math.min(120, duration - maxPrepMinutes); value -= (recent.get(recipe.id) || 0) * 60; value -= Math.abs((index % 7) - ((recipe.title || '').length % 7)); return value; }
...
module.exports = { FAMILY_PORTIONS, seasonFor, datesBetween, attendanceFor, generatePlan, recalculatePlanShopping, scaledIngredients, score };
