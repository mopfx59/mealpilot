...
function expandEvents(events = []) {
  ...
  for (const event of events) {
    ...
    for (const date = new Date(`${start}T12:00:00`); date <= end; date.setDate(date.getDate() + 1)) result.push({ id: event.id, date: dayKey(date.toISOString()), title: event.summary || '', type: classifyEvent(event.summary) });
  }
  return result;
}
...
async function fetchZoneBHolidays(startDate, endDate, fetchImpl = fetch) {
  ...
  const data = await response.json(); return (data.results || []).map(item => ({ name: item.description, start: dayKey(item.start_date), end: dayKey(item.end_date) }));
}
