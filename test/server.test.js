...
test('importe une sélection puis permet de supprimer toute recette définitivement', async t => {
  ...
  response = await send(base, `/api/recipes/${imported.id}`, 'PUT', { ...imported, title: 'Recette adaptée', seasons: ['Automne', 'Hiver'], ingredients: [{ name: 'courgettes', quantity: 3, unit: 'pièces' }], preparation: ['Découper', 'Cuire doucement'] }); const edited = await response.json(); assert.equal(response.status, 200); assert.deepEqual(edited.seasons, ['Automne', 'Hiver']); assert.equal(edited.ingredients[0].quantity, 3); assert.equal(edited.preparation.length, 2); assert.equal(edited.source, 'Source test');
...
test('enregistre les habitudes et exceptions individuelles des enfants', async t => {
  const base=await setup(t); const payload={children:[{name:'Alice',birthDate:'2020-09-08',canteenDays:[1,2,4,5],centreDays:[3]},{name:'Tom',birthDate:'2023-01-01',canteenDays:[2,4],centreDays:[]}],exceptions:[{date:'2026-09-08',memberId:'child2',lunch:'home',dinner:'outside'}],specialDays:[{date:'2026-09-10',type:'canteenClosed',label:'Grève'}]};
  const response=await send(base,'/api/family-schedule-v2','PUT',payload); assert.equal(response.status,200); const schedule=await response.json(); assert.equal(schedule.children[0].name,'Alice'); assert.equal(schedule.children[0].birthDate,'2020-09-08'); assert.deepEqual(schedule.children[1].canteenDays,[2,4]); assert.equal(schedule.exceptions[0].dinner,'outside'); assert.equal(schedule.specialDays[0].label,'Grève');
});
