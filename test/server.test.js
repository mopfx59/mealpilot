...
test('importe une sélection puis permet de supprimer toute recette définitivement', async t => {
  ...
  response = await send(base, `/api/recipes/${imported.id}`, 'PUT', { ...imported, title: 'Recette adaptée', season: 'Automne', ingredients: [{ name: 'courgettes', quantity: 3, unit: 'pièces' }], preparation: ['Découper', 'Cuire doucement'] }); const edited = await response.json(); assert.equal(response.status, 200); assert.equal(edited.season, 'Automne'); assert.equal(edited.ingredients[0].quantity, 3); assert.equal(edited.preparation.length, 2); assert.equal(edited.source, 'Source test');
...
test('conserve et met à jour le statut d’un reste planifié', async t => {
  const base = await setup(t);
  const response = await send(base, '/api/plan/generate', 'POST', { startDate: '2026-08-03', endDate: '2026-08-04', soloMadame: ['2026-08-04'], busyDates: [], lunchMaxMinutes: 60, dinnerMaxMinutes: 45, busyMaxMinutes: 30 });
